...
function Avaliacao({name, children, funcao}) {
    return (
        <article className="sm:m-6 my-4 p-6 w-full bg-white rounded-sm border-t-2 border-sky-700 shadow-lg">
            <div className="flex items-center mb-4 space-x-4">
                <img className="w-10 h-10 rounded-full" src={pic} alt="Imagem de perfil" />
                <div className="space-y-1 font-medium ">
                    <span>
                        {name}
                        <div className="block text-sm text-gray-500 ">{funcao}</div>
                    </span>
                </div>
            </div>
            ...
            <span className="mb-2 font-light text-gray-500 ">{children}</span>
        </article>
    );
}
